---  
layout: post  
title: Flutter Battery app with Klutter
---  
...
- In a terminal run: **flutter create --org com.example --template=plugin --platforms=android,ios batterylevel**{: style="color: Cyan; opacity: 0.80;" }
- Install the Klutter Framework as dependency:
  - Add klutter: ^0.1.2 to your pubspec.yaml dependencies block.
  - In a terminal run: **flutter pub get**{: style="color: Cyan; opacity: 0.80;" }
- In a terminal run: **flutter pub run klutter:producer init**{: style="color: Cyan; opacity: 0.80;" }
...
This class is our platform interface. We have a BatterLevel class which has a constructor that requires an object which
...
Just like the <b>expect</b> keyword, actual is a Kotlin Multiplatform feature. The actual classes in iosMain and androidMain
...
We also see an annotation: <B>@KlutterAdaptee("getBatteryLevel")</B>. This annotation tells Klutter to generate method-channel
code for this method and add it to the Flutter library file.

Finally we convert the Float to a Double value.

# Step 3: Build the Flutter library
- Open a terminal in the root folder and run: **flutter pub run klutter:producer install=platform**{: style="color: Cyan; opacity: 0.80;" }
- In a terminal run: **flutter pub run klutter:producer install=library**{: style="color: Cyan; opacity: 0.80;" } 
...
In a terminal run: **flutter pub run klutter:consumer add=batterylevel**{: style="color: Cyan; opacity: 0.80;" }
...
Congratulations on your first Klutter plugin! 